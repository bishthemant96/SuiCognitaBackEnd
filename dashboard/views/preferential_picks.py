...
#MainClass
class PreferentialPicks(APIView):
    def get(self, request, id):
        user_tuple = Users.objects.get(id=id)
        preferences = []
        preferences.append(user_tuple.preference1)
        preferences.append(user_tuple.preference2)
        preferences.append(user_tuple.preference3)

        movie_match_set = GenresMovies.objects.filter(genre_id__in=preferences)

        movie_set = []
        for item in movie_match_set:
            movie_set.append(item.movie_id)


        good_ratings_set = Ratings.objects.filter(movie_id__in=movie_set, rating__range=[4,6])
        bad_ratings_set = Ratings.objects.filter(movie_id__in=movie_set, rating__range=[-1,2])
